    ...
    <div class="container mx-auto px-4 py-8">
      ...
      <div class="mt-8 grid grid-cols-1 lg:grid-cols-4 gap-8">
        ...
        <div class="lg:col-span-1">
          <DocsSidebar :categories="categories" v-model="activeCategory" />
        </div>
        ...
        <div class="lg:col-span-3">
          ...
          <div v-else class="space-y-8">
            <div v-for="category in displayCategories" :key="category.category">
              <h2 :id="category.category.toLowerCase()" class="text-2xl font-bold text-gray-900 dark:text-gray-100 mb-4 scroll-mt-24">
                {{ category.category }}
              </h2>
...
import type { ClassData, CategoryData } from '~/types/classes'
import Fuse from 'fuse.js'

useSeoMeta({
  title: 'Documentation - PascalCSS Class Reference',
  description: 'Complete searchable reference for all PascalCSS utility classes. Find the perfect class for your needs.',
...
// Load classes data
const { data: classesData } = await useFetch<CategoryData[]>('/data/classes.json')
